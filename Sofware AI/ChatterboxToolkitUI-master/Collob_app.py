...
def yield_tts_updates(log_msg=None, audio_data=None, file_list=None, log_append=True):
    ...
    # ✅ Updated logic: show both audio and files if both are provided
    audio_update = gr.update(value=audio_data, visible=(audio_data is not None))
    files_update = gr.update(value=file_list if file_list else None, visible=(file_list is not None and len(file_list) > 0))
...
# === NEW RESTART BUTTON SECTION - ADD AT THE END ===
with gr.Blocks(title="ChatterboxToolkitUI", theme=gr.themes.Default()) as demo:
    gr.Markdown(
        """
        # ChatterboxToolkitUI
    ...
    # === NEW RESTART BUTTON SECTION WITH ANIMATION ===
with gr.Blocks(title="ChatterboxToolkitUI", theme=gr.themes.Default()) as demo:
    gr.Markdown(
        """
        # ChatterboxToolkitUI
    ...
    with gr.Tabs(elem_id="main_tabs_id") as main_tabs:
        
        # --- Main Tab 1: Single Generation ---
        with gr.Tab("Single Generation", id="single_generation_main_tab"):
            # ... your existing single generation code ...
            
        # --- Main Tab 2: Batch Generation ---  
        with gr.Tab("Batch Generation"):
            # ... your existing batch generation code ...
    ...
    # === NEW: ANIMATED RESTART BUTTON SECTION ===
    with gr.Row():
        with gr.Column(scale=1):
            gr.Markdown("### 🔄 Session Control Center")
            with gr.Row():
            ...
            restart_status = gr.Textbox(
                ...
                value="✅ Session is running normally",
                interactive=False
            )

    # === RESTART FUNCTION ===
    def restart_colab_session():
        import os
        import time
        
        status = "🚨 INITIATING RESTART SEQUENCE...\n"
        status += "3... 2... 1...\n" 
        status += "🔄 RESTARTING GOOGLE COLAB SESSION\n"
        status += "Please wait for session to reload automatically..."
